...
The `home-routes` handler in the `actman.routes.home` namespace
...
The `home-page` function will in turn call the `actman.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `actman.handler` namespace:
...
Request middleware functions are located under the `actman.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `actman.dev-middleware` namespace found in
